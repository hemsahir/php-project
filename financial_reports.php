...
$lang = $_SESSION['lang'] ?? 'hi'; // assume language stored in session
$categories = ['Budget' => 'बजट', 'Audit Report' => 'ऑडिट रिपोर्ट', 'Balance Sheet' => 'बैलेंस शीट'];

// Fetch data grouped by category
$documents = [];
$filterCategory = $_GET['category'] ?? '';
$searchTitle = $_GET['title'] ?? '';
foreach ($categories as $key => $title) {
    if ($filterCategory && $filterCategory !== $key) continue;
    $sql = "SELECT * FROM financial_documents WHERE category = '$key'";
    if ($searchTitle) {
        $search = $conn->real_escape_string($searchTitle);
        $sql .= " AND title LIKE '%$search%'";
    }
    $sql .= " ORDER BY id DESC";
    $res = $conn->query($sql);
    $documents[$key] = $res->fetch_all(MYSQLI_ASSOC);
}
?>
...
<head>
  <meta charset="UTF-8">
  <title><?= $lang === 'hi' ? 'बजट एवं ऑडिट रिपोर्ट' : 'Budget & Audit Report' ?></title>
        ...
        <div class="container common-container four_content">
            ...
            <div class="col-md-9 col-sm-12">
                <form method="GET" class="row g-3 align-items-center mb-4">
                    ...
                    <div class="col-md-2">
                        <a href="financial_reports.php" class="btn btn-secondary w-100">
                ...
                $labels = [
                    ...
                    'Audit Report' => ['hi' => 'ऑडिट रिपोर्ट', 'en' => 'Audit Report'],
                    'Balance Sheet' => ['hi' => 'बैलेंस शीट', 'en' => 'Balance Sheet']
                ];
                foreach ($documents as $category => $items):
                    if (count($items) > 0):
                ?>
                <div class="section-header"><?= $labels[$category][$lang] ?></div>
                ...
                <table class="table table-bordered align-middle">
                    ...
                    <tr>
                        <td><?= $i + 1 ?></td>
                        <td class="doc-title"><?= htmlspecialchars($title) ?></td>
                        <td>
